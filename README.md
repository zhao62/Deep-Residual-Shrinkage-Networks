...
**The paper has been cited over 1300 times on Google Scholar. (本论文的谷歌学术引用量已经超过1300次)**
...
[1143] 杨嘉诚.基于激光诱导击穿光谱的金属材料成分和硬度分析研究[D].福建理工大学,2024.

>本文使用含有软阈值结构的**深度残差收缩网络**（**Deep Residual Shrinkage Networks**,**DRSN**）进行成分分析。

[1144] 朱宽.基于深度学习的无线电调制识别研究[D].四川大学,2021.

>针对缺失信号先验信息，人工选取特征操作复杂的问题，研究出一种基于多尺度特征融合的**残差收缩神经网络**调制识别算法。

[1145] 王延臣,张宏成,张啸尘.基于双域特征融合的轴承故障诊断方法分析与研究[J].沈阳建筑大学学报(自然科学版),2025,41(02):271-279.

>采用**残差收缩网络**提取原始信号的特征，采用卷积网络，并加入注意力机制，提取时频图中的频域信息

[1146] 杨恒.基于时序收缩网络和NetVLAD的单词级视听语音识别研究[D].天津大学,2022.

>在该架构中，本文首先基于**深度残差收缩网络**结构提出了时序收缩单元，然后将多尺度时序卷积网络中的基础单元替换为时序收缩单元，最后采用NetVLAD层以替换全局平均池化。

[1147] 顾士兴.**残差收缩**的可分辨ViT故障诊断算法研究[D].辽宁大学,2024.

[1148] 厦门炬研电子科技有限公司.一种改进的**深度残差收缩网络**:202411754354.5[P].2025-03-21.

[1149] 中南大学.一种基于直接吸收光谱的甲烷浓度测量方法及测量系统:202411503081.7[P].2025-03-21.

>对**深度残差收缩网络**模型进行改进得到改进后的深度残差收缩网络模型，并构建甲烷浓度测量模型；

[1150] 哈尔滨工业大学.基于时频分析和深度学习的地震相位拾取方法:202411801323.0[P].2025-03-14.

>第二部分是由多个**残差收缩模块**串联组成，残差收缩模块是在传统残差模块的基础上添加了一个软阈值函数，用于滤除背景噪声；

[1151] 国网宁夏电力有限公司超高压公司,国网宁夏电力有限公司.基于非接触式传声器阵列的换流阀故障监测方法:202411564986.5[P].2025-03-07.

>将基准mel谱特征作为训练样本以训练**深度残差收缩网络**模型，利用训练后的**深度残差收缩网络**模型识别当前输入数据，以确定当前换流阀的声音信号数据是否为异音信号数据...

[1152] 东风悦享科技有限公司,东风汽车集团股份有限公司.一种分布式多载体跨平台的自动驾驶数据采集系统:202411786683.8[P].2025-03-07.

>采用基于**深度残差收缩网络**的故障诊断算法对自动驾驶数据的采集进行故障诊断，输出自动驾驶数据的采集的故障数据信息

[1153] 矿业大学(北京)内蒙古研究院,中国矿业大学(北京).矿井突水识别方法、装置、电子设备及存储介质:202411403875.6[P].2025-03-04.

>并且该初始模型包括**残差收缩层**和卷积块注意力层，增强了对水流特征的关注和背景干扰的抑制，显著提升了在水流边缘检测和复杂背景分割中的性能。

[1154] 中国科学院自动化研究所.基于多维度变换神经网络模型的血压测量方法和装置:202510039096.0[P].2025-04-11.

>所述多维度变换神经网络模型包括基于最值的维度变换块、基于快速傅里叶变换的维度变换块、和一维**深度残差收缩网络**。

[1155] 杭州极弱磁场国家重大科技基础设施研究院.脑电信号的处理方法、装置以及脑电极帽:202411858660.3[P].2025-04-11.

>基于**深度残差收缩网络**模型对脑电信号的残留脑电成分进行特征提取，并将特征信息与脑电信号的主体成分进行重建；

[1156] 东北石油大学三亚海洋油气研究院.基于迁移学习和改进残差网络复合材料损伤识别方法:202510220687.8[P].2025-03-28.

>用梅尔频谱倒谱系数图谱数据集训练，得到训练好的基于迁移学习的**深度残差收缩网络**模型；使用训练好的基于迁移学习的**深度残差收缩网络**模型,对待识别复合材料损伤类型进行识别。

[1257] 中国自然资源航空物探遥感中心.一种基于MS-1DCNN-DRS网络模型的高光谱岩性填图方法:202411608347.4[P].2025-03-25.

>并通过加入**深度残差收缩网络**，减少高光谱影像的噪声影响以及剔除不同波段之间的相关性较高的重复信息。

[1158] 樊越,颜拥军.基于改进**DRSN**算法的半导体探测器故障诊断研究[J/OL].核电子学与探测技术,2025,(06):1-13[2025-04-25].

>本文尝试了一种基于改进型**深度残差收缩网络**(**DRSN**)算法的半导体探测器故障诊断方法研究

[1159] 陈仁祥,张雁峰,杨黎霞,等.基于多尺度改进深度残差收缩网络的滚动轴承寿命状态识别[J].振动与冲击,2025,44(07):217-224.

>提出基于多尺度改进**深度残差收缩网络**(multi scale-improved deep residual shrinkage network, MS-IDRSN)的滚动轴承寿命状态识别方法。

[1160] 陈伟,陈名德,郭锐,等.基于**残差收缩网络**的地震数据增强方法[J/OL].长江大学学报(自然科学版),1-11[2025-04-25].

[1161] 储焰,常远,汤何胜.基于精细复合多元多尺度散布熵和**深度残差收缩网络**的轴向柱塞泵故障诊断[J].机床与液压,2025,53(06):142-147.

[1162] 王玉静,李祎然,康守强,等.基于数字孪生的不同工况下谐波减速器故障诊断方法[J/OL].机械工程学报,1-15[2025-04-25].

>引入改进的半软阈值函数构建**深度残差收缩网络**，抑制噪声干扰并提取特征

[1163] 梁红琴,姜进南,龙辉,等.基于**深度残差收缩网络**的地铁车轮扁疤故障诊断[J].中南大学学报(自然科学版),2025,56(03):1234-1248.

[1164] 张颖,赵祺旸,郗群.基于深度融合网络研究糖尿病视网膜病变[J].中国医学物理学杂志,2025,42(03):347-355.

>使用Inception-Resnet-V2作为主网络，并融入**深度残差收缩网络**和金字塔分割注意力模块，有效地过滤掉特征学习过程中的无关信息

[1165] 刘杨远,沈龙江,贺世忠,等.基于**深度残差收缩网络**的地铁车辆转向架轴承故障诊断[J].电力机车与城轨车辆,2025,48(02):40-46.

[1166] 刘高辉,张宇.领域知识驱动结合深度学习的调制识别方法[J/OL].小型微型计算机系统,1-9[2025-04-25].

>送入结合软阈值降噪算法的**深度残差收缩网络**提取频域特征

[1167] 陈文壮,刘昕明,毛爱坤,等.基于IDRSN-MADA轴承变工况故障诊断模型[J/OL].控制工程,1-8[2025-04-25].

>提出一种基于改进**深度残差收缩网络**与多对抗域自适应结合的轴承故障双流诊断模型

[1168] 周苗苗,郭海伟,刘鹏飞,等.基于改进**深度残差收缩网络**的轴承疲劳寿命评估研究[J].机械设计与研究,2025,41(01):333-336+344.

[1169] 杨朋辉,杨长青,刘静,等.基于2D-3D卷积神经网络的情绪识别模型[J].燕山大学学报,2025,49(01):66-73.

>在网络中引入SE-ResNet网络、**深度残差收缩网络**和Xception网络

[1170] 李振业,周静雷,路昌.基于VMD和CBAM-**DRSN**的扬声器异常声分类方法研究[J].信息技术与信息化,2025,(01):116-120.

>其次，利用**深度残差收缩网络**提取深层次特征
...
[1178] 董春峰,张岩,刘小秋,等.基于改进U-Net的微地震事件识别方法[J].吉林大学学报(地球科学版),2025,55(03):970-986.

>该方法在U-Net下采样过程中引入**残差收缩模块**，通过残差块实现网络结构的跨层连接以减少模型训练时特征信息的损失，结合软阈值收缩技术削弱微地震数据中噪声特征的干扰，增强模型的抗噪能力；

[1179] 李红旭,卢益涛,迟荣华,等.融合**深度残差收缩**和轻量化Transformer编码技术的点云分割模型[J].激光与光电子学进展,2025,62(10):161-172.

[1180] 朱海龙,董绍江,赵兴新,等.基于多尺度自适应残差网络的轴承故障诊断[J].组合机床与自动化加工技术,2025,(05):189-193+200.

>对于**深度残差收缩网络**模块，通过引入可变核卷积，根据输入数据的变化自主进行动态改变卷积核大小，引入concat连接，使深层特征和浅层特征进一步融合，利用注意力模块和软阈值对逐通道特征赋不同阈值并降噪。

[1181] 万志国,赵伟,王治国,等.基于优化VMD-NLM与1D**DRSN**的齿轮箱故障诊断方法[J].机械传动,2025,49(05):150-160.

[1182] 曹毅,王彦雯,李杰,等.基于减小高频混响和RF-**DRSN**-EMA的声音事件分类方法[J/OL].华南理工大学学报(自然科学版),1-10[2025-06-04].

[1183] 王新,唐灿,朱建新,等.基于深度增强与特征抗噪的夜间串番茄成熟度识别方法[J].农业机械学报,2025,56(04):363-374.

>同时引入**深度残差收缩网络**，在YOLO v5s中构建RSBottleneck-CW模块对特征图进行软阈值化处理,抑制图像中的噪声干扰。

[1184] 潘雪.强干扰下通信信号调制识别技术研究[D].电子科技大学,2024.

>本文基于Tensorflow框架构建了端到端的调制识别网络模型，包括卷积神经网络(CNN)和**深度残差收缩网络**(**DRSN**)模型

[1185] 中国石油化工股份有限公司,中国石油化工股份有限公司胜利油田分公司.一种基于**深度残差收缩网络**的储层裂缝指数预测方法:202311335140.X[P].2025-04-18.

[1186] 北京航空航天大学.一种空间非合作动态目标进近视觉探测方法:202510017599.8[P].2025-04-18.

>采用**深度残差收缩网络**实现强噪声干扰环境下目标检测；

[1187] 张逸凡.基于深度学习的光伏阵列故障诊断方法研究[D].武汉大学,2022.

>建立以二维波形图像为输入样本的**深度残差收缩网络**(Deep Residual Shrinkage Network,DRSN)模型,用于并网光伏阵列故障诊断。

[1188] 曹一菡.基于深度学习的航天器关键部件寿命预测[D].南京航空航天大学,2023.

>利用XGboost筛选与轴承寿命强相关的特征,借助**深度残差收缩网络**对不同强度噪声的抗干扰能力,建立特征与寿命的映射关系。

[1189] 张清智.基于深度学习的水声目标检测研究[D].桂林电子科技大学,2023.

>空间域注意力机制模块,采用残差网络和**残差收缩网络**相结合的结构,在图谱能量增强和轨迹关联方面取得良好的折中。

[1190] 杨紫佳.基于深度学习的农作物害虫检测及识别方法研究[D].浙江农林大学,2023.

>在**深度残差收缩网络**中引入空间注意力机制模块,提高常见农作物害虫图像分类的正确率。

[1191] 周超.基于TimesBlock和对比学习的多场景轴承故障诊断方法研究[D].大连理工大学,2024.

>提出了TB-MCNN结合**残差收缩模块**的多信息融合RS-TBMCNN故障诊断模型。

[1192] 孙磊.复杂工况下滚动轴承智能故障诊断算法研究[D].大连理工大学,2023.

>接着引入残差连接、注意力机制和软阈值对卷积神经网络进行改进,搭建**深度残差收缩网络**(DRSN)

[1193] 章嘉湾.基于多模态时序信号的自动睡眠分期研究[D].大连理工大学,2024.

>通过引入**深度残差收缩网络**自适应地调整通道间特征依赖关系,有效降低了噪声特征对模型性能的影响。
...
[1220] 廖星.深度学习在数字信号调制识别中的应用研究[D].四川大学,2023.

>使用**残差收缩网络**对不同的信号样本进行自适应软阈值降噪,挖掘被噪声淹没的特征,以此来完成调制分类。

[1221] 李帅.煤矿井下基于**深度残差收缩网络**的可见光定位算法研究[D].内蒙古科技大学,2025.

[1222] 韩东洋.基于FSST和MFDRSN的滚动轴承故障诊断研究[D].郑州大学,2023.

>提出基于同步压缩短时傅里叶变换(SST-based-STFT,FSST)和多尺度特征融合**深度残差收缩网络**(Multi-scale Feature fusion **Deep Residual Shrinkage Network**,MFDRSN)的轴承故障诊断方法。

[1223] 周康慧.噪声环境下基于IDRSN与ConvGRU的车轴疲劳裂纹声发射信号研究[D].大连交通大学,2025.

>将卷积注意力模块（CBAM）嵌入基于稠密连接的**深度残差收缩网络**（DBDRSN）,构建CBAM-DBDRSN模型,强化对关键特征的捕捉能力。

[1224] 郑虎.基于双流神经网络的滚动轴承故障诊断研究[D].安徽工业大学,2023.

>引入**深度残差收缩网络**,提出了一种基于二维灰度图像的**深度残差收缩网络**轴承故障诊断算法

[1225] 金钰寒.复杂道路监控下的多目标车辆跟踪检测方法研究[D].大连交通大学,2025.

>将YOLOv8中的C2f模块替换为**深度残差收缩网络**中的DRSN-CS模块,增强了特征提取能力

[1226] 北京航空航天大学.一种用于强噪声干扰环境下的视觉目标检测方法及装置:202510516350.1[P].2025-07-18.

>添加**深度残差收缩模块**DRSB；在YOLOv8的骨干网络中,将C2f模块替换为**深度残差收缩模块**DRSB,对原YOLOv8的骨干网络进行改进；

[1227] 郭正刚,孙磊,王贺,等.基于FSWT-**DRSN**的滚动轴承故障诊断方法[J].机械设计与制造工程,2025,54(08):77-82.

[1228] 王强,吕鸿浩,周小超,等.基于I**DRSN**的高噪声环境下小电流接地系统故障选线方法研究[J/OL].高压电器,1-17[2025-08-28].

>针对现有选线方法抗噪能力不足的问题，文中提出一种基于改进**深度残差收缩网络**（improved **deep residual shrinkage network**，I**DRSN**）的故障选线新方法。

[1229] 张冠杰,李艳斌,畅鑫,等.基于**DRSN**-Transformer编码器的域自适应辐射源个体识别方法研究[J].河北工业科技,2025,42(04):303-313.

[1230] 刘保罗,李晨,聂雅琳,等.基于**残差收缩**卷积和GSoP注意力机制的旋转机械故障诊断[J].振动与冲击,2025,44(13):277-287.

[1231] 周静雷,李振业,路昌,等.结合不均衡样本生成及BOA-**DRSN**的扬声器异常声分类[J].西安工程大学学报,2025,39(04):37-45.

[1232] 李啸天,常家盛,侯艳丽,等.基于稠密残差注意力网络的调制方式识别[J/OL].计算机测量与控制,1-11[2025-08-28].

>2.1基于稠密连接的**残差收缩模块**

[1233] 杨开元,李天昀,曹思钰,等.基于混合神经网络的CPM信号调制指数估计方法[J].信息工程大学学报,2025,26(03):253-260.

>该方法在经典的深度残差网络的基础上引入软阈值化和上下文自注意力机制构成上下文**残差收缩网络**（CRSNet）

[1234] 田荣坤,秦玉华,张金凤,等.基于改进**DRSN**的近红外光谱特征选择方法研究[J].光散射学报,2025,37(02):265-275.

[1235] 韩亚旭.面向电力调度场景的语音识别技术研究与应用[D].内蒙古科技大学,2025.

>本文提出一种结合**深度残差收缩网络**(**DRSN**)和Conformer网络的电力调度语音识别声学模型DRformer

[1236] 段德春.基于深度学习的CSI室内定位方法研究[D].内蒙古科技大学,2025.

>针对上述问题,本文提出基于逐天线与子载波的**深度残差收缩网络**(**Deep Residual Shrinkage Network** with Antennas-wise and Subcarriers-wise thresholds,**DRSN**-AS)的室内定位方法,该网络通过内嵌软阈值化模块实现信号降噪

[1237] 樊飞.基于轻量化网络的光学遥感图像无锚目标检测算法研究[D].内蒙古科技大学,2025.

>提出了优化的**残差收缩网络**来消除多尺度特征融合中的背景噪声和冲突信息

[1238] 胡硕.基于多源数据融合的转子不平衡故障诊断方法研究[D].郑州轻工业大学,2025.

>提出一种基于改进**残差收缩块**(Improved **Residual Shrinkage Block Unit**,IRSBU)与多源数据融合小波核网络(Multi-Source Data Fusion Wavelet Kernel Network,MSWKN)的诊断方法

[1239] 王思思.变速工况轴承故障在线智能诊断及RUL预测研究[D].中南林业科技大学,2025.

>提出基于**深度残差收缩网络**和时间卷积网络的预测模型。

[1240] 王震.基于改进UNet模型的图像去雨算法研究[D].重庆三峡学院,2025.DOI:10.27883/d.cnki.gcqsx.2025.000202.

>提出了一种基于双注意力密集**残差收缩网络**的图像去雨算法

[1241] 陈增旭.基于深度学习的滚动轴承复合故障诊断研究[D].黑龙江八一农垦大学,2025.

>还提出了基于改进**深度残差收缩网络**(**DRSN**)与门控循环单元(GRU)的双通道抗噪模型,并结合一维振动信号与多标签分类技术,有效提升了诊断准确率和抗噪声能力

[1242] 吴飞.基于知识图谱的电力通信系统故障诊断方法研究与实现[D].东北电力大学,2025.

>使用基于自注意力机制的图卷积网络和**残差收缩网络**介入图消息传递过程,增强模型对长距离依赖关系的捕捉和对噪声的抑制

[1243] 王逸风.双轴硅晶圆划切装备刀具状态监测及控制软件研究[D].东华大学,2025.

>提出了一种一维多层**深度残差收缩网络**的刀具破损检测模型

[1244] 吴雨凡.功能性聚酯纤维聚合过程多源特征迁移个性化软测量建模[D].东华大学,2025.

>针对聚酯纤维聚合过程工业变量呈现的高动态性、高噪声特性,提出基于**残差收缩机制**的软测量基模型。

[1245] 宋其璋.氦质谱检漏仪控制系统及关键技术研究[D].电子科技大学,2025.

>首先基于软阈值化的思想设计了**深度残差收缩**自编码器网络DRSAN,之后分析了小波变换、LSTM和DRSAN的优劣势,最终提出了一种结合了**深度残差收缩**自编码器机制和LSTM的协同去噪网络D-LSDN,并在真实噪声数据集上进行测试

[1246] 罗乐.基于振动信号的行星轮系齿轮裂纹辨识[D].重庆理工大学,2025.

>提出了一种基于双通道**深度残差收缩神经网络**(Double Channel-Deep Residual Shrinkage Network,DC-DRSN)的状态辨识方法

[1247] 张印文.基于深度学习的滚动轴承故障诊断与预测研究[D].沈阳航空航天大学,2024.

>本研究提出了一种结合**深度残差收缩网络**(**deep residual shrinkage network**,**DRSN**)、双向长短期记忆网络(bidirectional long short term memory,BiLSTM)与Transformer的模型

[1248] 何坤.基于机器学习的航空轴承故障诊断方法研究[D].中国民航大学,2024.

>之后将提取到的特征信息进行卷积操作并导入到改进**残差收缩网络模块**中进行深层次的特征挖掘,最后基于全连接层完成对故障的分类

[1249] 陈闻鹤.基于深度学习的风电机组运行状态监测方法研究[D].南京理工大学,2024.

>提出基于降噪通道卷积注意力模块的多通道不同阈值**深度残差收缩网络**(CAMDRSN-CW)

[1250] 刘加林.基于多任务学习的多目标跟踪方法研究[D].江南大学,2024.

>该方法采用基于多通道**残差收缩网络**来提取不同子任务的关键信息,以确保不同任务的差异性优化

[1251] 梅青山.基于lMS谱图和MobileNet网络的机器异常声音检测方法研究[D].桂林理工大学,2024.

>针对工业生产过程中环境噪声较大,机器异常声音特征无法被充分学习的问题,提出了一种基于**深度残差收缩网络**的机器异常声音检测方法

[1252] 王小聪.基于深度学习的锂电池SOC及SOH联合预测方法研究[D].贵州大学,2024.

>该模型融合了**深度残差收缩网络**(**Deep Residual Shrinkage Networks** with Channel-wise Thresholds,**DRSN**-CW)与时序型长短期记忆神经网络(Long-Short Term Mermory network,LSTM)

[1253] 卞雨辰.基于深度学习的长输管道漏磁缺陷识别方法研究[D].东北石油大学,2024.

>针对目标分类任务,本文提出了一种将注意力机制(AMs)与**深度残差收缩网络**(**DRSNs**)相结合的改进方法来提高漏磁异常图像识别的准确性和高噪声环境下的抗干扰能力

[1254] 王传浩.基于卷积神经网络的轴承故障诊断研究[D].济南大学,2024.

>研究了一种基于连续小波变换、通道注意力机制、**残差收缩模块**的双路径卷积神经网络的智能故障诊断模型。首先,构建了双通道特征提取机制,分别以信号和时频图像作为输入。然后,**残差收缩模块**作为信号特征提取的一条路径,具有通道注意力机制作为另一条路径。

[1255] 袁文涛.基于深度学习的肌电手势识别研究[D].南京林业大学,2024.

>模型中引入了改进的**残差收缩网络模块**,该模块采用注意力机制来计算合成信号图像中每个特征通道的权重,并应用软阈值函数对信号特征进行滤波和重构,从而提高了关键手势特征的识别能力

[1256] 董春峰.基于改进U-Net网络的微地震事件识别方法研究[D].东北石油大学,2024.

>提出融合注意力门和**残差收缩**U-Net网络的事件识别方法

[1257] 张志伟.基于机器视觉的圩堤溃口水速测量研究[D].南京林业大学,2024.

>在特征提取层采用了集成残差结构、注意力机制与软阈值的**残差收缩模块**(Residual Shrinkage Building Unit with Channel-Wise Thresholds,RSBU-CW)进行重构,实现不同特征通道的自适应处理,提高模型鲁棒性

[1258] 马媛媛.基于深度学习的RIS辅助多用户通信系统信道估计技术研究[D].山东大学,2024.

>搭建**残差收缩网络**(**Deep Residual Shrinkage Networks**,**DRSN**)对预估计信道矩阵进行降噪处理,利用**DRSN**的非线性特性,学习输入信号中的复杂噪声特征,并通过调整训练网络中的权重参数来生成降噪后的输出

[1259] 董乾霖.基于模型压缩和数据降维的射频指纹特征提取与识别技术研究[D].海南大学,2024.

>提出了一种基于权重剪枝和知识蒸馏的**深度残差收缩网络**框架下的射频指纹特征提取与识别方法。

[1260] 段阁飞.三维集成电路中TSV缺陷测试方法研究[D].桂林电子科技大学,2024.

>本文提出了一种微变恒流源以及结合格拉姆角场(Gram Angle Field,GAF)图像编码与**深度残差收缩网络**(**Deep Residual Shrinkage Network**,**DRSN**)的TSV绑定前故障缺陷无损检测方法

[1261] 刘豪.基于多源域迁移学习的轴承故障诊断方法研究[D].河南科技大学,2024.

>构建了一种基于金字塔挤压注意力**残差收缩网络**(PSADRSN)的多源域对抗故障诊断方法

[1262] 张彦彦.基于小波变换和深度神经网络的电能质量扰动分析[D].华北电力大学(北京),2024.

>提出了基于密集连接的改进**深度残差收缩网络**,形成复杂噪声下的一体化扰动识别模型

[1263] 刘宇鑫.基于改进卷积神经网络的数模融合变压器剩余寿命预测[D].华北电力大学(北京),2024.

>提出一种结合改进卷积神经网络和**深度残差收缩网络**的电力变压器退化特征提取方法

[1264] 胡皓.复杂工况下旋转机械故障诊断方法研究[D].燕山大学,2024.

>针对旋转机械多工况数据分布不均衡导致故障分类准确率较低的问题,研究基于鲸鱼优化变分模态分解与**深度残差收缩网络**的多工况故障诊断方法

[1265] 周锦龙.基于边缘计算的船舶推进轴系健康监测系统设计与开发[D].华中科技大学,2024.

>提出基于**深度残差收缩网络**的船舶推进轴系故障诊断算法,通过搭建船舶推进轴系实验台对所提出的故障诊断算法有效性进行验证

[1266] 张潇然.基于深度时序特征学习的滚动轴承故障诊断与剩余寿命预测研究[D].北京工业大学,2024.

>针对滚动轴承运行数据含噪声干扰导致诊断精度降低的问题,提出一种基于自适应双通道去噪多尺度注意力**残差收缩网络**(Multiscale Attention **Residual Shrinkage Network** with Adaptive Dual-Channel Denoising,AMARSN)的故障诊断方法,提取具有强判别性的多尺度故障特征,实现噪声干扰下滚动轴承故障的精准识别

[1267] 王锐琦.基于优化深度强化学习的永磁同步电机匝间短路故障诊断方法[D].江苏科技大学,2024.

>针对Dueling DQN对故障信息特征提取能力不足和对经验样本抽取效率不高的问题,本文提出了一维分层**深度残差收缩**-优先经验回放-竞争深度Q网络(One-Dimensional Group **Deep Residual Shrinkage Network**-Prioritized Experience Replay-Dueling DQN,1DGDRSN-PER-Dueling DQN)的故障诊断方法

[1268] 刘统国.基于深度神经网络的滚动轴承故障诊断研究[D].兰州理工大学,2024.

>该模块使用深度可分离卷积以降低网络参数量,同时进一步提取局部特征和全局特征,引入**残差收缩单元**以提高网络的抗噪性能;然后,设计了空间信息融合模块

[1269] 马立东.风力发电机组远程运维系统研发[D].兰州理工大学,2024.

>研究中还提出了一种基于**深度残差收缩网络**(**DRSN**-CW)的智能故障诊断方法,同时结合了上述联合降噪策略

[1270] 姚瑞芳.MIMO-FTN无线光通信系统中改进型神经网络检测算法研究[D].兰州理工大学,2024

>在**深度残差收缩网络**(**Deep Residual Shrinkage Network**,**DRSN**)的基础上,结合Inception模块提出了一种改进型DRSN(improved IDRSN,IDRSN)检测算法

[1272] 王晨明.基于数据增强和改进**深度残差收缩网络**的电力变压器故障诊断研究[D].宁夏大学,2024.

[1273] 周嘉豪.基于深度学习的UUV推进器与传感器故障诊断研究[D].哈尔滨工程大学,2024.

>本文设置了两种类型的网络架构,一种是所有特征通道共享相同阈值的**深度残差收缩网络**(DRSNCS),另一种是所有特征通道分配不同阈值的**深度残差收缩网络**(DRSN-CW)。

[1274] 山东海纳智能装备科技股份有限公司.一种矿用高压变频器故障分析诊断方法及系统:202510983469.X[P].2025-08-15.

>将特征值输入预训练的**深度残差收缩网络**诊断模型进行多模态融合分析,输出故障风险评估结果

[1275] 天津商业大学.变工艺铣削加工粗糙度预测方法及系统:202510553351.3[P].2025-08-15.

>根据所述**深度残差收缩网络**的残差块和注意力机制,将所述数据样本转化为第一特征；

[1276] 中国人民解放军国防科技大学.雷达辐射源个体识别方法、装置、计算机设备和存储介质:202510952465.5[P].2025-08-12.

>利用改进型**深度残差收缩网络**,对所述预处理后脉冲序列进行局部特征提取,得到局部特征

[1277] 佛山市顺德区美的洗涤电器制造有限公司.噪音去除的方法、装置、电子设备及介质:202110639632.2[P].2025-07-04.

>将所述待识别语音数据输入至预设的编解码模型,得到噪音识别结果,所述编解码模型为基于目标软阈值化函数生成的目标**深度残差收缩网络**以及特征还原网络所构成

[1278] 南京航空航天大学.一种基于深度学习的航空发动机转子系统故障诊断方法:202111237432.0[P].2025-07-04.

>建立新型**深度残差收缩网络**进行特征提取,并将首层卷积层设为宽卷积核,使用渐进半软阈值函数算法作为网络的收缩层

[1279] 电子科技大学.一种变工况下列车传动系统故障检测与定位方法、系统:202510372955.8[P].2025-06-27.

>利用**残差收缩特征**提取模块剔除数据中的噪声干扰,提取代表性特征

[1280] 三峡大学.考虑时空特性和风速-功率转换机理的风电功率预测方法:202510219283.7[P].2025-06-13.
